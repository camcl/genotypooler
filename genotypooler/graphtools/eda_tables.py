...
# Plot other MAF vs. true MAF

truemaf = df0['maf'].to_frame()

# if df2 is not None:
#     pooledbin = pd.cut(df2.true_af.values.squeeze(), x_bins, labels=lab_fmt, include_lowest=True)
#     df2['AF-bin'] = pooledbin
#     pooledmaf = df2['maf'].to_frame()
# 
#     dfmaf = truemaf.join(pooledmaf, how='right', lsuffix='_true', rsuffix='_pooled')
#     # binmaf = truemaf.join(pooledmaf, how='right', lsuffix='_true', rsuffix='_pooled')
#     print(df0.true_af)
#     figaf, axmaf = plt.subplots(nrows=1, ncols=1)
#     dfmaf.plot(x='maf_true', y='maf_pooled',
#                s=5,
#                kind='scatter', ax=axmaf)
#     axmaf.set_xlabel('MAF in true data')
#     axmaf.set_ylabel('MAF in pooled data')
#     plt.savefig(os.path.join(outdir, 'markers-maf-true-pooled.pdf'))
#     plt.show()


# Plot founders MAF vs. inbred MAF

if df3 is not None:
    plt.rcParams["figure.figsize"] = [16, 4]
    #otherbin = pd.cut(df3.af.values.squeeze(), x_bins, labels=lab_fmt, include_lowest=True)
    #df3['AF-bin'] = otherbin
